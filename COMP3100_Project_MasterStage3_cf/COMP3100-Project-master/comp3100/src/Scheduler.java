...
public class Scheduler{
        ...
        while(!getCommand(msg).equals("NONE")){
            ...
            if(getCommand(msg).equals("JOBN") || getCommand(msg).equals("JOBP")){
                ...
                if(getCommand(msg).equals("DATA")){
                    ...
                    float cumFit = Float.MAX_VALUE;
                    boolean found= false;
                    
                    ServerInfo server1 = null;
                    while(!getCommand(msg).equals(".")){
                        server = getServerInfo(msg);
                       //System.out.println("cserver state  " + Integer.parseInt(server.state) + "server corecount  " + Integer.parseInt(server.coreCount));
                        ...
                        if(Integer.parseInt(server.state) < 4 && Integer.parseInt(server.coreCount) > 0 ){
                            ...
                            float memFit =  (Integer.parseInt(server.memory) -Integer.parseInt(job.memory))/1000000;
                            //System.out.println("coreFit  " + coreFit + "memFit  " + memFit +);
                            // int diskFit = Integer.parseInt(server.disk) -Integer.parseInt(job.disk)
                            if(coreFit >= 0 && memFit >= 0){// && diskFit >= 0){
                                float newFit = coreFit + memFit;// + diskFit;
                                if (cumFit > newFit){
                                   found = true;
                                   cumFit = newFit;
                                   server1 = server;
                    ...
                    if(found == true){
                        ...
                        System.out.println("cf - job scheduled    " + n+" " + job.id + " " + job.cores + " " + job.memory + " " + job.estRunTime);
                        n=n+1;
                    }
                    else{
                        nxtj();
                        /*System.out.println("cf - job NOT scheduled    " + n+" " + job.id + " " + job.cores + " " + job.memory + " " + job.estRunTime);                   
                        try {
                                int inChar = System.in.read();
                            }
                        catch (IOException e){System.out.println("Error reading from user");}*/
                        //kilj(server.type, server.id, job.id);
                    }
                }            
            }
    ...
    public void cumulativeFit1(ArrayList<Server> servers){
        System.out.println("CumulativeFit1");
        String msg = readFromStream();
        ...
        Job job = null;
            ...
            if(getCommand(msg).equals("JOBN") || getCommand(msg).equals("JOBP")){
                ...
                if(getCommand(msg).equals("DATA")){
                    ...
                    if(found == true){
                        schd(job.id, server1.type, server1.id);
                        System.out.println("cf1 - job scheduled    " + n+" " + job.id + " " + job.cores + " " + job.memory + " " + job.estRunTime);
                        n=n+1;
                    }
                    else{
                        nxtj();
                        System.out.println("cf1 - job NOT scheduled    " + n+" " + job.id + " " + job.cores + " " + job.memory + " " + job.estRunTime);                   
                    }
                }
            }
            msg = readFromStream();
    ...
    public void cumulativeFit2(ArrayList<Server> servers){
        System.out.println("cumulativeFit2");
        String msg = readFromStream();
        ...
        while(!getCommand(msg).equals("NONE")){
            ...
            if(getCommand(msg).equals("JOBN") || getCommand(msg).equals("JOBP")){
                ...
                if(getCommand(msg).equals("DATA")){
                    ...
                    if(found == true){
                        schd(job.id, server1.type, server1.id);
                        System.out.println("cf2 - job scheduled    " + n+" " + job.id + " " + job.cores + " " + job.memory + " " + job.estRunTime);
                        n=n+1;
                    }
                    else{
                        nxtj();
                        //System.out.println("cf2 - job NOT scheduled    " + n+" " + job.id + " " + job.cores + " " + job.memory + " " + job.estRunTime);                   
                    }
                }
            }
            msg = readFromStream();
    ...
    public void cumulativeFit3(ArrayList<Server> servers){
        System.out.println("cumulativeFit3");
        String msg = readFromStream();
                   ...
                   float cumFit = Float.MAX_VALUE;
                    ...
                    while(!getCommand(msg).equals(".")){
                        ...
                        if(Integer.parseInt(server.state) < 4 && Integer.parseInt(server.coreCount) > 0 ){
                            ...
                            if(Integer.parseInt(server.state) != -1 && Integer.parseInt(server.availableTime) >= Integer.parseInt(job.estRunTime)){
                            //if(Integer.parseInt(server.availableTime) >= Integer.parseInt(job.estRunTime)){
                            //if(Integer.parseInt(server.state) != -1 ){
                                int coreFit = Integer.parseInt(server.coreCount) -Integer.parseInt(job.cores);
                    ...
                    if(found == true){
                        schd(job.id, server1.type, server1.id);
                        System.out.println("cf3 - job scheduled    " + n+" " + job.id + " " + job.cores + " " + job.memory + " " + job.estRunTime);
                        n=n+1;
                    }
                    else{
                        nxtj();
                        //System.out.println("cf3 - job NOT scheduled    " + n+" " + job.id + " " + job.cores + " " + job.memory + " " + job.estRunTime);                   
                    }
                }
            }
            msg = readFromStream();
